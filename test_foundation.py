...
ctx = Ctx()
Powertrain(ctx)
ok("Pmax = 456000 W", ctx.pt.Pmax == 456000.0)
ok("Tmax = 602 Nm", ctx.pt.Tmax == 602.0)
ok("OMmax = 17750*pi/30", abs(ctx.pt.OMmax - 17750 * np.pi / 30) < 1e-9)
ok("Vmax = 80.555 m/s", abs(ctx.pt.Vmax - 290 / 3.6) < 1e-9)
ok("Rw = 0.3142857", ctx.vp.Rw == 0.3142857)
