...
Builds the OCP and collocation options onto the shared ``ctx`` (after loading the
powertrain and vehicle params). Config is exposed as function args with the same
defaults as the MATLAB file (AeroConfig='Static', ATD='On', Electric_4Motors='Off',
circuit='BCN', vi=60, ni=nan).

Linear solver defaults to HSL ma57; MUMPS (casadi-bundled, no external library) is
the fallback.
"""
...
def userOpts(ctx,
    ...
    # `linear_solver` default 'ma57' (Coin-HSL, loaded at solve time from
    # `hsl_dir` / COINHSL_DIR / seeded default in functions/hsl.py); _make_solver
    # probes it and falls back to MUMPS (casadi-bundled, always available) if HSL
    # is unavailable.
    ipopt = {
        "max_iter": 6000,
